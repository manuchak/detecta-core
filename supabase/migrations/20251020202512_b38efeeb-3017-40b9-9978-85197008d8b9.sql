-- Corregir la función get_analyst_assigned_leads_v2 para usar programacion_llamadas en lugar de scheduled_calls
...
BEGIN
  ...
  SELECT 
    ...
    l.notas as notas,
    CASE 
      WHEN sc.id IS NOT NULL AND sc.estado = 'programada' THEN true
      ELSE false
    END as has_scheduled_call,
    sc.fecha_programada as next_scheduled_call
  FROM public.leads l
  LEFT JOIN public.lead_approval_process lap ON l.id = lap.lead_id
  LEFT JOIN public.programacion_llamadas sc ON l.id::text = sc.lead_id 
    AND sc.estado = 'programada'
    AND sc.fecha_programada >= now()
  WHERE (
