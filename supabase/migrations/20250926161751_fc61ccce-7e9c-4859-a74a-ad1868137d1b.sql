-- Corregir la función RPC verificar_disponibilidad_equitativa_custodio
-- Problema: busca 'custodio_nombre' en servicios_planificados cuando debe ser 'custodio_asignado'

CREATE OR REPLACE FUNCTION public.verificar_disponibilidad_equitativa_custodio(p_custodio_id uuid, p_custodio_nombre text, p_fecha_servicio date, p_hora_inicio time without time zone, p_duracion_estimada_horas integer DEFAULT 4)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Contar servicios asignados hoy en AMBAS tablas
  -- Servicios en servicios_custodia (históricos/existentes)
  ...
  -- CORRECCIÓN: Usar custodio_asignado en lugar de custodio_nombre
  -- Sumar servicios planificados (nuevos)
  ...
  WHERE (sp.custodio_id = p_custodio_id OR sp.custodio_asignado = p_custodio_nombre)
    AND DATE(sp.fecha_servicio) = p_fecha_servicio
    AND sp.estado IN ('asignado', 'confirmado', 'en_progreso');
  ...
  conflictos_planificados AS (
    SELECT 
      sp.id::text as id_servicio,
      (sp.fecha_servicio::date + sp.hora_servicio::time)::timestamp as fecha_hora_cita,
    ...
    WHERE (sp.custodio_id = p_custodio_id OR sp.custodio_asignado = p_custodio_nombre)
      ...
      AND (
        (p_hora_inicio::time, hora_fin) 
        OVERLAPS 
        (sp.hora_servicio::time, (sp.hora_servicio::time + COALESCE(sp.duracion_horas, 4) * interval '1 hour')::time)
      )
  ),
  todos_conflictos AS (
  ...
  horas_planificadas AS (
    -- CORRECCIÓN: Usar custodio_asignado en lugar de custodio_nombre
    SELECT COALESCE(SUM(COALESCE(sp.duracion_horas, 4)), 0) as horas
    FROM servicios_planificados sp
    WHERE (sp.custodio_id = p_custodio_id OR sp.custodio_asignado = p_custodio_nombre)
      AND DATE(sp.fecha_servicio) = p_fecha_servicio
      AND sp.estado IN ('asignado', 'confirmado', 'en_progreso')
  ...
  WITH ultimo_servicio AS (
    ...
    FROM (
      ...
      UNION ALL
      -- CORRECCIÓN: Usar custodio_asignado en lugar de custodio_nombre  
      SELECT MAX(sp.fecha_servicio) as fecha
      FROM servicios_planificados sp
      WHERE (sp.custodio_id = p_custodio_id OR sp.custodio_asignado = p_custodio_nombre)
        AND sp.estado IN ('asignado', 'confirmado', 'en_progreso')
    ) combined
