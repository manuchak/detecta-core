CREATE OR REPLACE FUNCTION public.liberar_custodio_a_planeacion_v2(p_custodio_liberacion_id uuid, p_aprobado_por uuid, p_forzar_liberacion boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  -- 1. Validar rol del usuario (FIX: leer de user_roles en vez de profiles)
  ...
  IF v_pc_custodio_id IS NULL THEN
    INSERT INTO pc_custodios (
      nombre, email, telefono, estado, vehiculo_propio
    ) VALUES (
