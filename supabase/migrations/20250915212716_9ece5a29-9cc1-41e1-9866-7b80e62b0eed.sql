-- Corregir el test para usar valores válidos de estado
...
BEGIN
  ...
  -- Esta tabla debe permitir que José Manuel vea solo sus propios datos
  BEGIN
  ...
  BEGIN
    -- Actualizar datos de progreso (como haría el auto-save)
  ...
  -- Test 7: Verificar que José Manuel puede leer sus propios datos de progreso
  BEGIN
    SELECT EXISTS(
      SELECT 1 FROM interview_progress 
      WHERE lead_id = test_lead_id 
        AND user_id = jose_user_id
        AND progress_data->>'final_decision' = 'approved'
    ) INTO test_result;
    
    result := result || jsonb_build_object('can_read_updated_progress', test_result);
  EXCEPTION WHEN OTHERS THEN
    result := result || jsonb_build_object('can_read_updated_progress', false, 'error', SQLERRM);
  END;

  -- Test 8: Verificar funcionalidad del formulario completo
  BEGIN
    -- Simular todos los campos que se guardarían en el formulario
    UPDATE interview_progress 
    SET progress_data = jsonb_build_object(
      'interviewNotes', 'Entrevista completa con José Manuel - Candidato aprobado',
      'decision', 'approved',
      'decisionReason', 'Excelente experiencia y conocimientos técnicos',
      'interviewer', 'José Manuel Muñoz Iglesias',
      'completed_at', now()::text,
      'form_version', '1.0'
    )
    WHERE lead_id = test_lead_id AND user_id = jose_user_id;
    
    result := result || jsonb_build_object('full_form_simulation', true);
  EXCEPTION WHEN OTHERS THEN
    result := result || jsonb_build_object('full_form_simulation', false, 'error', SQLERRM);
  END;

  -- Limpiar datos de prueba
  DELETE FROM interview_progress WHERE lead_id = test_lead_id;
  ...
  result := result || jsonb_build_object(
    'test_summary', jsonb_build_object(
      ...
      'role', 'supply_lead',
      'test_scenario', 'Complete interview form data saving workflow',
      'completed_at', now(),
      'conclusion', 'José Manuel puede guardar información de formularios de entrevista correctamente'
    )
  );
