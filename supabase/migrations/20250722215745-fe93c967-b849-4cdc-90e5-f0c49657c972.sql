-- Actualizar las funciones para usar períodos más específicos y datos más actuales
...
-- Función para distribución de ingresos - ÚLTIMOS 30 DÍAS
...
BEGIN
  RETURN QUERY
  WITH custodian_income_last_30_days AS (
    SELECT 
      sc.nombre_custodio,
      COUNT(*) as servicios_mes,
      SUM(CAST(COALESCE(NULLIF(sc.costo_custodio, ''), '0') AS numeric)) as ingresos_mes
    ...
    WHERE sc.costo_custodio IS NOT NULL 
      ...
      AND sc.fecha_hora_cita >= CURRENT_DATE - INTERVAL '30 days'
      AND sc.fecha_hora_cita <= CURRENT_DATE
    GROUP BY sc.nombre_custodio
    HAVING SUM(CAST(COALESCE(NULLIF(sc.costo_custodio, ''), '0') AS numeric)) > 0
  ),
  income_levels AS (
    SELECT 
      CASE 
      ...
      ingresos_mes
    FROM custodian_income_last_30_days
  ),
  total_count AS (
...
-- Función para métricas de activación - basada en datos históricos
...
BEGIN
  ...
  WITH activation_data AS (
    ...
    WHERE sc.fecha_contratacion IS NOT NULL 
      ...
      AND sc.nombre_custodio != ''
      AND sc.nombre_custodio != '#N/A'
      AND sc.fecha_contratacion >= CURRENT_DATE - INTERVAL '6 months'
    GROUP BY sc.nombre_custodio
    HAVING EXTRACT(days FROM (MIN(sc.fecha_hora_cita) - MIN(sc.fecha_contratacion))) >= 0
      AND EXTRACT(days FROM (MIN(sc.fecha_hora_cita) - MIN(sc.fecha_contratacion))) <= 60
  )
  SELECT 
    ...
    ROUND((COUNT(*) FILTER (WHERE dias_activacion <= 7)::numeric / NULLIF(COUNT(*), 0) * 100), 1) as tasa_activacion_rapida
  FROM activation_data;
END;
$function$;

-- Función para matriz de retención por cohortes - últimos 6 meses
...
BEGIN
  ...
  cohort_retention AS (
    ...
    HAVING COUNT(DISTINCT cc.nombre_custodio) >= 5  -- Solo cohortes con al menos 5 custodios
  )
  SELECT 
    TO_CHAR(cohorte_mes_date, 'YYYY-MM') as cohorte_mes,
    ROUND((activos_mes_0::numeric / NULLIF(custodios_iniciales, 0) * 100), 1) as mes_0,
    ROUND((activos_mes_1::numeric / NULLIF(custodios_iniciales, 0) * 100), 1) as mes_1,
    ROUND((activos_mes_2::numeric / NULLIF(custodios_iniciales, 0) * 100), 1) as mes_2,
    ROUND((activos_mes_3::numeric / NULLIF(custodios_iniciales, 0) * 100), 1) as mes_3,
    ROUND((activos_mes_4::numeric / NULLIF(custodios_iniciales, 0) * 100), 1) as mes_4,
    ROUND((activos_mes_5::numeric / NULLIF(custodios_iniciales, 0) * 100), 1) as mes_5,
    custodios_iniciales
...
-- Función para estadísticas de productividad mensual - últimos 6 meses
...
BEGIN
  ...
  WITH monthly_stats AS (
    ...
    FROM servicios_custodia sc
    WHERE sc.fecha_hora_cita >= CURRENT_DATE - INTERVAL '6 months'
      AND sc.fecha_hora_cita <= CURRENT_DATE
      AND sc.nombre_custodio IS NOT NULL
      AND sc.nombre_custodio != ''
      AND sc.nombre_custodio != '#N/A'
  ...
  SELECT 
    ...
    ROUND(AVG(NULLIF(ingresos_custodio, 0)), 0) as ingresos_promedio,
