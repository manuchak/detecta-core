-- Fix the migration function with correct UUID handling
...
BEGIN
  ...
  LOOP
    -- Verificar si ya existe un vehículo para este custodio (fixed: use UUID = UUID comparison)
    IF NOT EXISTS (
      ...
      BEGIN
        ...
        BEGIN
          ...
          IF custodio_id_found IS NOT NULL THEN
            -- Insertar vehículo (fixed: use UUID directly, not as text)
