-- SEMANA 1: Migración base para sistema de indisponibilidades

-- 1. Extender enum de disponibilidad para incluir temporalmente_indisponible
DO $$ 
BEGIN
    -- Verificar si el valor ya existe antes de agregarlo
    IF NOT EXISTS (
        SELECT 1 FROM pg_enum 
        WHERE enumlabel = 'temporalmente_indisponible'::text 
        AND enumtypid = 'disponibilidad_custodio'::regtype
    ) THEN
        ALTER TYPE disponibilidad_custodio ADD VALUE 'temporalmente_indisponible';
    END IF;
END $$;

-- 2. Crear tabla para gestionar indisponibilidades temporales
...
-- 4. Crear vista para custodios operativos disponibles (filtro de 90 días y nuevos)
...
-- 5. Función para auto-reactivar custodios cuando expire su indisponibilidad
...
-- 7. Trigger para actualizar disponibilidad del custodio automáticamente
...
-- 8. RLS Policies para la nueva tabla
