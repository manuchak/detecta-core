-- Fix UUID conversion bug in RPC functions
-- Remove ::uuid casts from lead_id parameters since leads.id and lead_approval_process.lead_id are text type

-- Fix update_approval_process function
...
BEGIN
  ...
  VALUES (
    p_lead_id, -- Removed ::uuid cast - keep as text
    current_user_id,
    p_stage,
    p_interview_method,
    p_notes,
...
-- Fix save_interview_session function
...
BEGIN
    ...
    SET 
        ...
        updated_at = now()
    WHERE id = p_lead_id; -- Removed ::text cast since it's already text
...
-- Fix mark_interview_interrupted function
...
BEGIN
    ...
    SET 
        ...
        updated_at = now()
    WHERE id = p_lead_id AND interview_session_id = p_session_id; -- Removed ::text cast since it's already text
    ...
    SET 
        ...
        updated_at = now()
    WHERE lead_id = p_lead_id; -- Removed ::text cast since it's already text
