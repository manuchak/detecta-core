-- Fix get_analyst_assigned_leads: change lead_id from uuid to text to match leads.id column type
-- This resolves error 42804: "Returned type text does not match expected type uuid in column 1"

-- First drop the existing function (required when changing RETURNS TABLE structure)
DROP FUNCTION IF EXISTS public.get_analyst_assigned_leads(integer, integer, timestamptz, timestamptz);

-- Recreate with correct return type (lead_id as text, not uuid)
...
RETURNS TABLE(
  lead_id text,  -- Changed from uuid to text to match leads.id column type
  lead_nombre text,
  lead_email text,
  lead_telefono text,
  lead_estado text,
