...
-- (Usando las columnas correctas de las tablas existentes)

-- 1. Insertar métricas de demanda de zona (usando columnas reales)
INSERT INTO public.metricas_demanda_zona (
  zona_id, periodo_inicio, periodo_fin, servicios_promedio_dia, custodios_activos, 
  custodios_requeridos, deficit_custodios, score_urgencia, gmv_promedio, 
  ingresos_esperados_custodio, created_at, updated_at
) VALUES 
  ('11111111-1111-1111-1111-111111111111', '2024-12-01', '2024-12-31', 45, 30, 45, 15, 85, 125000, 4500, now(), now()),
  ('22222222-2222-2222-2222-222222222222', '2024-12-01', '2024-12-31', 25, 17, 25, 8, 65, 75000, 3800, now(), now()),
  ('33333333-3333-3333-3333-333333333333', '2024-12-01', '2024-12-31', 60, 38, 60, 22, 95, 180000, 5200, now(), now()),
  ('44444444-4444-4444-4444-444444444444', '2024-12-01', '2024-12-31', 20, 15, 20, 5, 45, 55000, 3200, now(), now())
...
-- 5. Crear la función para generar alertas automáticas (si no existe)
