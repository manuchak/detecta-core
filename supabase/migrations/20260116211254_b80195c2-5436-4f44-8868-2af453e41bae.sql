-- ============================================================
-- MIGRACIÓN: RPC v2 canónica + wrapper legacy retrocompatible
-- FIX: DROP explícito antes de recrear con nombres diferentes
-- ============================================================

-- 1) DROP de la función existente PRIMERO (obligatorio para cambiar nombres de params)
DROP FUNCTION IF EXISTS public.liberar_custodio_a_planeacion(uuid, uuid, boolean);

-- 2) Crear la función v2 CANÓNICA con la lógica completa
...
-- 3) Crear wrapper LEGACY con parámetros p_liberacion_id/p_liberado_por (para clientes viejos)
...
AS $$
BEGIN
  -- Simplemente delegar a la v2 con los parámetros renombrados
  RETURN public.liberar_custodio_a_planeacion_v2(
    p_liberacion_id,
    p_liberado_por,
    p_forzar_liberacion
  );
...
-- Permisos para wrapper legacy
REVOKE ALL ON FUNCTION public.liberar_custodio_a_planeacion(uuid, uuid, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.liberar_custodio_a_planeacion(uuid, uuid, boolean) TO authenticated;