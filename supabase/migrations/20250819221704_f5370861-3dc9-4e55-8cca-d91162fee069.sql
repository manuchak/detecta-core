-- Crear o reemplazar la función para obtener solo los leads asignados al usuario actual
...
BEGIN
    ...
    -- Devolver solo los leads asignados al usuario actual
    RETURN QUERY
    SELECT 
    ...
    WHERE l.asignado_a = current_user_id  -- FILTRO CRÍTICO: Solo leads asignados al usuario actual
      AND l.estado NOT IN ('rechazado', 'aprobado') -- Excluir leads ya procesados (excepto pool de reserva)
    ORDER BY 
