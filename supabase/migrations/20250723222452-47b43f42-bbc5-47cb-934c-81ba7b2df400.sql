-- Corregir función RPC para administradores y columnas existentes
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(lap.last_session_id::text, '') as interview_session_id, -- Corregir nombre de columna
  ...
  WHERE (
    -- Si es administrador, mostrar TODOS los leads que tengan un analista asignado
    -- (no importa si está asignado a él o a otro analista)
    is_admin_user AND l.asignado_a IS NOT NULL
  ) OR (
    -- Si no es administrador, solo los asignados específicamente a él
