-- Fix: Include services with posicionamiento_falso=true even if canceled
-- These services should remain visible for custodian payment and client penalty tracking

CREATE OR REPLACE FUNCTION public.get_real_planned_services_summary(date_filter date)
 RETURNS TABLE(total_services bigint, assigned_services bigint, pending_services bigint, confirmed_services bigint, services_data jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WITH service_stats AS (
    ...
    WHERE DATE(sp.fecha_hora_cita AT TIME ZONE 'America/Mexico_City') = date_filter
      -- FIX: Include false positionings even if canceled
      AND (
        sp.estado_planeacion NOT IN ('cancelado', 'finalizado')
        OR sp.posicionamiento_falso = true
      )
  ),
  services_json AS (
    SELECT jsonb_agg(
      jsonb_build_object(
        ...
        'planner_name', COALESCE(p.display_name, p.email, ''),
        ...
        -- NEW: Expose false positioning fields
        'posicionamiento_falso', COALESCE(sp.posicionamiento_falso, false),
        'cobro_posicionamiento', COALESCE(sp.cobro_posicionamiento, false),
        'motivo_posicionamiento_falso', sp.motivo_posicionamiento_falso
      ) ORDER BY sp.fecha_hora_cita ASC
    ) as data
    FROM servicios_planificados sp
    LEFT JOIN public.profiles p ON sp.asignado_por = p.id
