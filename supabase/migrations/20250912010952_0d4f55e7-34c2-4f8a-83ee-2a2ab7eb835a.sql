-- Actualizar la función get_ytd_by_exact_dates para excluir servicios cancelados
CREATE OR REPLACE FUNCTION public.get_ytd_by_exact_dates(start_date_current date, end_date_current date, start_date_previous date, end_date_previous date)
 RETURNS TABLE(current_year integer, current_services integer, current_gmv numeric, previous_year integer, previous_services integer, previous_gmv numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
