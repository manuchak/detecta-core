-- Función corregida para ROI marketing con tipos correctos
...
RETURNS TABLE(
  ...
  candidatos_totales numeric,
  custodios_activos numeric
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  candidatos_marketing AS (
    SELECT 
      COUNT(DISTINCT id)::numeric as total_candidatos,
      COUNT(DISTINCT CASE 
        WHEN EXISTS (
      ...
      END)::numeric as custodios_activos
