-- Phase 3: Fix remaining functions with search_path corrections
-- Drop and recreate functions that have parameter name conflicts

-- Drop existing functions first
DROP FUNCTION IF EXISTS public.verificar_cumplimiento_referido(UUID);
DROP FUNCTION IF EXISTS public.procesar_bono_referido(UUID);
DROP FUNCTION IF EXISTS public.get_custodio_referral_stats(UUID);
DROP FUNCTION IF EXISTS public.get_custodio_referidos(UUID);
DROP FUNCTION IF EXISTS public.transaction_crear_aprobacion_coordinador(UUID, UUID, TEXT, JSONB);
DROP FUNCTION IF EXISTS public.update_role_permission_secure(UUID, BOOLEAN);
