...
-- (Con valores correctos para score_urgencia: 1-10)

-- 1. Insertar métricas de demanda de zona (con score_urgencia válido)
INSERT INTO public.metricas_demanda_zona (
  ...
  ('11111111-1111-1111-1111-111111111111', '2024-12-01', '2024-12-31', 45, 30, 45, 9, 125000, 4500, now(), now()),
  ('22222222-2222-2222-2222-222222222222', '2024-12-01', '2024-12-31', 25, 17, 25, 6, 75000, 3800, now(), now()),
  ('33333333-3333-3333-3333-333333333333', '2024-12-01', '2024-12-31', 60, 38, 60, 10, 180000, 5200, now(), now()),
  ('44444444-4444-4444-4444-444444444444', '2024-12-01', '2024-12-31', 20, 15, 20, 4, 55000, 3200, now(), now())
...
DECLARE
    ...
    score_urgente INTEGER := 8; -- Cambiado a 8 ya que el máximo es 10
BEGIN
