-- Fase 1: Corregir validate_multiple_service_ids - renombrar columna "exists" a "record_exists"
-- Esto soluciona el error "structure of query does not match function result type"
...
RETURNS TABLE(
  id_servicio text,
  record_exists boolean,  -- ✅ Cambiado de "exists" (palabra reservada SQL)
  is_finished boolean,
...
SET statement_timeout TO '30s'  -- ✅ Aumentado de 10s a 30s para archivos grandes
AS $$
DECLARE
  v_user_id uuid;
BEGIN
  ...
  SELECT 
    ...
    (sc.id_servicio IS NOT NULL) as record_exists,  -- ✅ Cambiado nombre
    CASE 
