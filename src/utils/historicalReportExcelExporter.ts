...
const createOperationalSheet = (data: HistoricalReportData['operational']): XLSX.WorkSheet | null => {
  ...
  const rows: (string | number)[][] = [
    ...
    [''],
    ['TOP 10 CUSTODIOS POR COBRO'],
    ['Nota: Solo incluye custodios con datos de costo registrados'],
    ['Rank', 'Nombre', 'Servicios', 'Meses Activos', 'Cobro Total', 'Promedio/Mes', 'GMV', 'Margen', 'Cobertura (%)'],
    ...data.topCustodians.map(c => [c.rank, c.name, c.services, c.mesesActivos, c.costoCustodio, c.promedioCostoMes, c.gmv, c.margen, c.coberturaDatos]),
    [''],
    ['TOP 10 CLIENTES'],
