...
-- AUDITORÍA FORENSE DE DATOS - ANÁLISIS MULTIDIMENSIONAL
-- Análisis exhaustivo de servicios enero-fecha actual
...
BEGIN
  ...
  SELECT 
    ...
    -- GMV calculations con diferentes filtros
...
-- 2. FUNCIÓN COMPARATIVA CON DASHBOARD
...
BEGIN
  ...
  SELECT 
    ...
    STRING_AGG(DISTINCT id_servicio || ' ($' || cobro_cliente::text || ')', ', ' LIMIT 5) as sample_data
  ...
  SELECT 
    ...
    STRING_AGG(DISTINCT id_servicio, ', ' LIMIT 5) as sample_data
  FROM public.servicios_custodia
  ...
  SELECT 
    ...
    STRING_AGG(DISTINCT nombre_custodio || ' (' || service_count::text || ')', ', ' LIMIT 3) as sample_data
  FROM (
    SELECT nombre_custodio, COUNT(*) as service_count
    FROM public.servicios_custodia
    WHERE fecha_hora_cita >= '2025-01-01'::timestamp with time zone
  ...
  SELECT 
    ...
    STRING_AGG(DISTINCT fecha_hora_cita::date::text, ', ' LIMIT 5) as sample_data
  FROM public.servicios_custodia
