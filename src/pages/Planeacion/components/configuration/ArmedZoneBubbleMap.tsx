/**
 * ArmedZoneBubbleMap - Visualiza la distribución de armados por zona base
...
import { Map, Info, AlertCircle, Shield } from 'lucide-react';
...
interface ArmedZoneBubbleMapProps {
...
// Colores para las burbujas (naranja/amber para armados)
const ZONE_COLORS = {
  high: 'hsl(25, 95%, 53%)',      // orange-500
  medium: 'hsl(32, 95%, 50%)',    // amber-500
  low: 'hsl(38, 92%, 50%)',       // yellow-500
...
export function ArmedZoneBubbleMap({ estadisticasZona, height = 'calc(var(--vh-full) - 280px)' }: ArmedZoneBubbleMapProps) {
  ...
  const maxArmados = zonasConGeo.length > 0 
    ? Math.max(...zonasConGeo.map(z => z.cantidad)) 
    : 1;
    
  const totalArmados = estadisticasZona.reduce((sum, [, count]) => sum + count, 0);
  ...
  const addBubbles = () => {
    ...
    zonasConGeo.forEach((zona, index) => {
      const sizeRatio = zona.cantidad / maxArmados;
      const minSize = 28;
      const maxSize = 65;
      ...
      markerRoot.className = 'armed-zone-marker-root';
      markerRoot.style.cssText = `width: ${size}px; height: ${size}px;`;
      
      const bubble = document.createElement('div');
      bubble.className = 'armed-zone-bubble';
      ...
      const porcentaje = Math.round((zona.cantidad / totalArmados) * 100);
      const popup = new mapboxgl.Popup({
        offset: [0, -size / 2 - 5],
        closeButton: false,
        closeOnClick: false,
        className: 'armed-zone-popup',
      }).setHTML(`
        <div style="padding: 8px; min-width: 130px;">
          ...
          <div style="font-size: 12px; color: #6b7280;">
            <div><strong>${zona.cantidad}</strong> armados</div>
      ...
      <div className="rounded-lg border bg-card">
        ...
        <div className="p-3">
          ...
          >
            ...
            <p className="text-xs mt-1">Asigna zonas a los armados para visualizar el mapa</p>
          </div>
        </div>
      </div>
    );
          ...
          <div className="flex gap-3 min-h-[320px]" style={{ height }}>
            ...
            <div className="w-52 flex-shrink-0 rounded-lg border bg-card p-3 overflow-y-auto">
              <div className="space-y-0.5 mb-3">
                ...
                <p className="text-xs text-muted-foreground">Armados por ubicación</p>
              </div>
                ...
                {sinZona > 0 && (
                  <div className="flex items-center gap-2 p-1.5 rounded-md bg-warning/5 border border-warning/20">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {Math.round((sinZona / totalArmados) * 100)}%
                      </div>
                    </div>
                  </div>
                )}
