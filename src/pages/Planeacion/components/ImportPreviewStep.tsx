...
import { importServicios, ImportProgress, ImportResult } from '@/services/importService';
...
export default function ImportPreviewStep({
  ...
  const handleStartImport = async () => {
    ...
    try {
      const result = await importServicios(transformedData, (progress) => {
      ...
      <div className="space-y-6 py-6">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            {importProgress?.message || 'Procesando registros...'}
          </p>
        </div>
        ...
        {importProgress && (
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span>Progreso: {importProgress.current} de {importProgress.total}</span>
      ...
      <div className="space-y-6 py-6">
        <div className="text-center">
          ...
          <h3 className="text-lg font-semibold mb-2">
            {importResult.success ? 'Importación Completada' : 'Importación Completada con Errores'}
          </h3>
          <p className="text-muted-foreground">
            {importResult.imported} registros importados correctamente
            {importResult.failed > 0 && `, ${importResult.failed} registros fallaron`}
          </p>
        </div>
            ...
            <AlertDescription>
              <div className="space-y-1">
                ...
                {importResult.errors.length > 5 && (
                  <div className="text-sm">... y {importResult.errors.length - 5} errores más</div>
                )}
              </div>
            </AlertDescription>
          </Alert>
