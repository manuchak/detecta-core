  ...
  const {
    ...
    queryFn: async () => {
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        // Filtrar aprobados que también aparecen en candidatos del mes
        const aprobadosDelMes = leadsDelMes.filter(candidato => 
          custodiosActivosEnMes.has(candidato.nombre)
        ).length;
