...
export function ProjectionsReportSection({ data }: ProjectionsReportSectionProps) {
  ...
  const forecastVsReal = data?.forecastVsReal || [];
  const annualProjection = data?.annualProjection || { optimistic: 0, expected: 0, conservative: 0 };
  const modelPrecision = data?.modelPrecision || { mapePromedio: 0, desviacionEstandar: 0 };
      ...
      <Card>
        ...
        <CardContent>
          {forecastVsReal.length > 0 ? (
            ...
            <p className="text-muted-foreground text-center py-4">No hay datos de forecast disponibles</p>
          )}
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-green-50 dark:bg-green-950/30 p-4 rounded-lg text-center">
              ...
              <p className="text-2xl font-bold text-green-600">{annualProjection.optimistic.toLocaleString()}</p>
            </div>
            <div className="bg-blue-50 dark:bg-blue-950/30 p-4 rounded-lg text-center">
              ...
              <p className="text-2xl font-bold text-blue-600">{annualProjection.expected.toLocaleString()}</p>
            </div>
            <div className="bg-orange-50 dark:bg-orange-950/30 p-4 rounded-lg text-center">
              ...
              <p className="text-2xl font-bold text-orange-600">{annualProjection.conservative.toLocaleString()}</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <p className="text-2xl font-bold">{modelPrecision.mapePromedio.toFixed(1)}%</p>
            </div>
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">Desviación Estándar</p>
              <p className="text-2xl font-bold">{modelPrecision.desviacionEstandar.toFixed(1)}%</p>
            </div>
          </div>
        </CardContent>
      </Card>
