    ...
    <section id={id} className="py-20 bg-gradient-to-b from-muted/30 to-background">
      <div className="container px-4 md:px-6">
        <div className="flex flex-col items-center space-y-4 text-center mb-12">
          <h2 className="text-3xl font-bold tracking-tight text-corporate-black sm:text-4xl md:text-5xl">Características que amarás</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-10 mt-12">
          {features.map((feature, index) => (
            <div key={index} className="flex gap-4 items-start">
              <div className="rounded-full bg-corporate-red/10 p-3 mt-1 flex-shrink-0">
                {feature.icon}
              </div>
              <div>
                <h3 className="text-xl font-bold mb-2 text-corporate-black">{feature.title}</h3>
