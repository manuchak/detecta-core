...
export const ClientServiceDonut = () => {
  const { clientsMTD, loading } = useExecutiveMultiYearData();

  if (loading) {
    return (
      <Card className="h-full">
        <CardHeader className="pb-2"><CardTitle className="text-sm font-medium flex items-center gap-2"><Users className="h-4 w-4" />Servicios por Cliente</CardTitle></CardHeader>
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-medium flex items-center gap-2">
            <Users className="h-4 w-4" />Servicios por Cliente MTD
          </CardTitle>
                  ...
                  if (active && payload?.length) {
                    ...
                    return (
                      <div className="bg-popover border border-border rounded-lg p-3 shadow-lg">
                        ...
                        <p className="text-sm text-muted-foreground">{d.value} servicios ({pct}%)</p>
                      </div>
                    );
                  }
                  return null;
