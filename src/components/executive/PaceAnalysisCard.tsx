...
export const PaceAnalysisCard = () => {
  ...
  const monthPaceStatus = monthData.currentPace >= monthData.requiredPace;
  const growthPaceStatus = monthData.currentPace >= growthPaceNeeded;
  const annualPaceStatus = currentAnnualPace >= requiredPaceFor2024;
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <span className="font-medium">{growthPaceNeeded}/día</span>
                {growthPaceStatus ? (
                  <CheckCircle className="h-4 w-4 text-success" />
                ) : (
                  <XCircle className="h-4 w-4 text-destructive" />
        ...
        <div className="border-t pt-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <span className="font-medium">{Math.round(currentAnnualPace * 10) / 10} servicios/día</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-muted-foreground">Para igualar 2024:</span>
              <div className="flex items-center gap-2">
                <span className="font-medium">{Math.round(requiredPaceFor2024 * 10) / 10}/día</span>
                <span className="text-xs text-muted-foreground">
                  (+{Math.round(((requiredPaceFor2024 / currentAnnualPace) - 1) * 100)}%)
                </span>
                {annualPaceStatus ? (
                  <CheckCircle className="h-4 w-4 text-success" />
                ) : (
