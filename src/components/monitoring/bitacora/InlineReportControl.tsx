...
interface InlineReportControlProps {
...
function parseCoords(input: string): { lat: number; lng: number } | null {
  // Google Maps link: .../@19.4326,-99.1332,...
...
export const InlineReportControl: React.FC<InlineReportControlProps> = ({ onSubmit, isPending, servicioId }) => {
  ...
  const handleSubmit = async () => {
    if (!descripcion.trim() && photos.length === 0) {
      toast.error('Agrega una descripción o foto');
      return;
    }
    setUploading(true);
    try {
  ...
  return (
    ...
    >
      <div className="flex gap-1.5">
        ...
        />
        <Input
          placeholder="Coords / Google Maps"
          value={coords}
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        {photos.map((p, i) => (
          <div key={i} className="relative h-10 w-10 rounded border overflow-hidden group">
        ...
        {photos.length < 5 && (
          <button
            className={cn(
              "h-10 w-10 rounded border border-dashed flex items-center justify-center text-muted-foreground hover:border-primary hover:text-primary transition-colors",
        ...
        <Button
          ...
          disabled={busy || (!descripcion.trim() && photos.length === 0)}
        >
